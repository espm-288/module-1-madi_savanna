...
top_emitters_2022 <- exio |>
    ...
    group_by(region) |>
    summarise(total_co2 = sum(value, na.rm = TRUE)) |>
    filter(grepl("^W", region)) |>  # Include only rest-of-world regions
    arrange(desc(total_co2)) |>
    head(5) |>
    collect()
...
# Create the time series plot
regions_plot <- ggplot(co2_regions_timeseries, aes(x = year, y = total_co2, color = region_name)) +
    geom_line(linewidth = 1.2) +
    geom_point(size = 2.5) +
    labs(
        title = "CO2 Emissions Over Time: Top 5 Emitting Regions",
...
# Save the plot
ggsave("co2_all_regions_timeseries.png", regions_plot, width = 10, height = 6, dpi = 300)
```
